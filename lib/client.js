...
var MODE_SHOWN = "shown";
var MODE_MORE = "more";
var MODE_HIDDEN = "hidden";
var MODES = [MODE_SHOWN, MODE_MORE, MODE_HIDDEN];
var zh = {
  "nav": "\u4FA7\u680F\u6309\u94AE\u8BBE\u7F6E",
...
function modeOf(visible, id) {
  ...
  if (v === true || v === void 0) return MODE_SHOWN;
  return v;
}
...
var PARK_ID = "dsh-sbf-park";
var escapeeState = /* @__PURE__ */ new Map();
var homes = /* @__PURE__ */ new WeakMap();
var escapeeWatchers = /* @__PURE__ */ new Set();
function isEscapee(id) {
  return PORTAL_HOSTS[id] !== void 0 || escapeeState.has(id);
}
...
function escapeeHost(id) {
  ...
  if (declared !== void 0 && declared.host !== void 0) {
...
function sendHome(id, node) {
  ...
  const declared = escapeeDeclaration(id);
  const anchor = declared !== void 0 && declared.homeBefore !== void 0 ? document.querySelector(declared.homeBefore) : null;
...
function applyEscapee(id) {
  ...
  if (node === null) return;
  const row = state.row;
  const target = state.mode === MODE_SHOWN ? null : state.mode === MODE_MORE && row !== null ? row : parkContainer();
  if (target === null) {
    sendHome(id, node);
    return;
  }
...
function setEscapeeMode(id, mode) {
  ...
  if (state === void 0 || state.mode === mode) return;
  state.mode = mode;
  applyEscapee(id);
}
function setEscapeeRow(id, row) {
  const state = escapeeRecord(id);
  if (state === void 0 || state.row === row) return;
  state.row = row;
  applyEscapee(id);
}
function escapeeRecord(id) {
  ...
  if (PORTAL_HOSTS[id] === void 0) return void 0;
...
function makeMirror(id, origComponent, fallbackIndex) {
  const Mirror = (props) => {
    ...
    (0, import_react.useEffect)(() => watchEscapees(() => setTick((v) => v + 1)), []);
    ...
    (0, import_react.useEffect)(() => {
      if (escapee) {
        ...
        const t12 = setTimeout(replay, 150);
        const t22 = setTimeout(replay, 1200);
        return () => {
          clearTimeout(t12);
          clearTimeout(t22);
        };
      }
      if (mode !== MODE_SHOWN) return void 0;
      const check = () => {
        discoverEscapee(id, cellRef.current);
      };
      check();
    ...
    if (escapee) return (0, import_react.createElement)(origComponent, props);
...
function MoreButton(props) {
  ...
  const hidden = listButtons2().filter((b) => modeOf(visible, b.id) === MODE_MORE);
  (0, import_react.useEffect)(() => {
    if (!open) return;
      ...
      return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
        "div",
        {
          ...
          children: [
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "dsh-sbf-mode", role: "group", "aria-label": labelOf(t, b.id, b.entry), children: MODES.map((m) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
              "button",
              {
                type: "button",
                ...
                "aria-pressed": mode === m,
                onClick: () => {
                  actions.setMode(b.id, m);
                },
                children: t(`settings.mode.${m}`)
              },
              m
            )) })
          ]
...
function makePopupMirror(id, origComponent) {
  const PopupMirror = (props) => {
    ...
    const escapee = isEscapee(id);
    (0, import_react.useEffect)(() => {
      if (!inMore) return void 0;
      if (escapee) {
        setEscapeeRow(id, rowRef.current);
        return () => {
          setEscapeeRow(id, null);
        };
      }
      const check = () => {
        discoverEscapee(id, rowRef.current);
      };
      check();
      const t1 = setTimeout(check, 300);
      return () => {
    ...
    if (escapee) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "dsh-sbf-popRow", ref: rowRef });
    return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "dsh-sbf-popRow", ref: rowRef, children: (0, import_react.createElement)(origComponent, props) });
  };
